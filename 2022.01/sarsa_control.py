'''
Sarsa control algorithm for blackjack
...
def main(trials=700000, lmbda=1, alpha=1e-2, epsilon=1.5e-1):
    ...
    for _ in tqdm(range(trials)):
        ...
        while s:
            ...
            aprime = max(_ACTIONS, key=lambda x: action_values[sprime, x])
            if random.random() < epsilon:
                aprime = not aprime
            # Now we have s, a, r, sprime, aprime
            action_values[s, a] += alpha * (
                r +
                lmbda * action_values[sprime, aprime] -
                action_values[s, a])
            s, a = sprime, aprime
